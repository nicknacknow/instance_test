...
#define EXTERNAL_MODE FALSE // set this to true if using in an EXE, false for DLL
...
public:
    ...
    void start(DWORD protection = 0)
    {
        ...
        VirtualProtect(reinterpret_cast<void*>(address), size, newProtect | PAGE_GUARD, &nothing);
    }

    void restore()
    {
        VirtualProtect(reinterpret_cast<void*>(address), size, lastProtect, &lastProtect);
    }
};
...
namespace memcheck
{
    ...
    void writeBytes(int at, void* value, size_t count)
    {
            memcpy(reinterpret_cast<void*>(at), value, count);
    }
    ...
    void placeHook(int at, int func, size_t nops)
    {
        ...
        if (proc == nullptr)
        {
            VirtualProtect(reinterpret_cast<void*>(at), 5 + nops, 0x40, &old);
            ...
            *(DWORD*)(at + 1) = rel;
            ...
            VirtualProtect(reinterpret_cast<void*>(at), 5 + nops, old, &old);
        }
        else {
            VirtualProtectEx(proc, reinterpret_cast<void*>(at), 5 + nops, 0x40, &old);
            ...
            VirtualProtectEx(proc, reinterpret_cast<void*>(at), 5 + nops, old, &old);
        }
    }
        ...
        case EXCEPTION_GUARD_PAGE:
        {
            if (Breakpoints.size() > 0)
            {
                for (Breakpoint* bp : Breakpoints)
                {
                    ...
                    VirtualQuery(reinterpret_cast<void*>(bp->address), &mbi, 0x2C);
    ...
    void loadHeaders()
    {
        if (proc == nullptr)
        {
            ...
            text_start = (int)getBaseModule(proc);
        }

        int at = text_start;
        while (!vmp0 && !vmp1)
        {
            if (readInt(at) == 0x30706D76) // "vmp0"
            {
                at += 0xB;
                vmp0 = text_start + readInt(at);
            }
            else if (readInt(at) == 0x31706D76) // "vmp1"
            {
                at += 0xB;
                vmp1 = text_start + readInt(at);
            }
            else {
                at++;
            }
        ...
        if (proc == nullptr)
        {
            VirtualQuery(reinterpret_cast<void*>(text_start + 0x1000), &mbi, 0x2C);
        }
        else
        {
            VirtualQueryEx(proc, reinterpret_cast<void*>(text_start + 0x1000), &mbi, 0x2C);
        }
    ...
    int clone_section(uintptr_t start, size_t* length)
    {
        ...
        else
        {
            ...
            BYTE* buffer = readBytes(start, mbi.RegionSize);
            writeBytes(sectionClone, buffer, mbi.RegionSize);
        }
    ...
    UINT bypassHiddenCheck(int silentchecker)
    {
        ...
        *(DWORD*)(patch + size) = rel;
        ...
        writeBytes(patch_address, &patch, size);
    ...
    UINT bypassMemcheckCore(int memcheck_hook_start)
    {
        ...
        DWORD size = 0;
        BYTE patch[512];
        BYTE* bytesRead = readBytes(memcheck_hook_start, 512);
        DWORD at = (DWORD)bytesRead;
    ...
    int loadSpoofer()
    {
        ...
        *(BYTE*)(at++) = 0x00;
        ...
        {
            printf("[Spoofer] Failed to allocate memory\n");
            system("PAUSE");

            return 0;
        }

        writeBytes(spoofer_address, &spoofer, at - (int)spoofer);
    ...
    void flush()
    {
        if (proc == nullptr)
        {
            VirtualFree(reinterpret_cast<void*>(spoof), 0, MEM_RELEASE);
            for (int patch : patches)
            {
                VirtualFree(reinterpret_cast<void*>(patch), 0, MEM_RELEASE);
            }
        }
        else {
            VirtualFreeEx(proc, reinterpret_cast<void*>(spoof), 0, MEM_RELEASE);
            for (int patch : patches)
            {
                VirtualFreeEx(proc, reinterpret_cast<void*>(patch), 0, MEM_RELEASE);
            }
        }
    }
}